...
def plot_metrics_bars(results, output_dir):
    ...
    map_scores = [results[system]['map'] for system in systems] # This one is likely correct if 'map' is a direct key
    ...
    # Use 'precision_at_k' from the results structure
    ...
    # Use 'recall_at_k' from the results structure
    ...
    # Use 'average_retrieval_time' from the results structure
...
def plot_precision_recall_curve(results, output_dir): # This function seems to be missing from your provided file, but if you add it:
...
def plot_metrics_by_k(results, output_dir):
    ...
    # Get k values
    k_values = sorted([int(k) for k in results[list(results.keys())[0]]['precision_at_k'].keys()]) # Adjusted
    
    # Plot precision@k
    plt.subplot(2, 2, 1)
    for system in results:
        precision_values = [results[system]['precision_at_k'][str(k)] for k in k_values] # Adjusted
    ...
    plt.subplot(2, 2, 2)
    for system in results:
        recall_values = [results[system]['recall_at_k'][str(k)] for k in k_values] # Adjusted
    ...
    plt.subplot(2, 2, 3)
    for system in results:
        f1_values = [results[system]['f1_score_at_k'][str(k)] for k in k_values] # Adjusted
...
def plot_radar_chart(results, output_dir):
    ...
    systems = list(results.keys())
    metrics = ['MAP', 'Precision@10', 'Recall@10', 'F1@10', 'Speed'] # Speed is 1/time
    ...
    # Calculate max_values for normalization correctly
    ...
    max_values_norm = [max_val_map, max_val_p10, max_val_r10, max_val_f10, max_val_speed]
    ...
    for system in systems:
        ...
        values = [
            results[system]['map'],
            results[system]['precision_at_k']['10'],    # Adjusted
            results[system]['recall_at_k']['10'],     # Adjusted
            results[system]['f1_score_at_k']['10'], # Adjusted
            speed                                     # Adjusted (using calculated speed)
        ]
...
def create_performance_table(results, output_dir):
    ...
    for k in k_values:
        prec_data = {'System': systems, f'Precision@{k}': [results[s]['precision_at_k'][str(k)] for s in systems]} # Adjusted
        dfs.append(pd.DataFrame(prec_data))
    
    for k in k_values:
        recall_data = {'System': systems, f'Recall@{k}': [results[s]['recall_at_k'][str(k)] for s in systems]} # Adjusted
        dfs.append(pd.DataFrame(recall_data))
    
    for k in k_values:
        f1_data = {'System': systems, f'F1@{k}': [results[s]['f1_score_at_k'][str(k)] for s in systems]} # Adjusted
        dfs.append(pd.DataFrame(f1_data))
    
    time_data = {'System': systems, 'Retrieval Time': [results[s]['average_retrieval_time'] for s in systems]} # Adjusted
