...
class LegalDeepseekAnswerer:
    ...
    def build_prompt(self, query, context_docs):
        """
        Build a prompt for the DeepSeek model with context management.
        """
            ...
            for attempt in range(retries):
                try:
                    ...
                    if attempt < retries - 1:
                        ...
                        print(f"❌ All retries failed for DeepSeek: {e}")
...
if __name__ == "__main__":
    ...
    # Note: For DeepSeek implementation, we'll do one hyperparameter grid search per question
    for idx, row in df.iterrows():
        ...
        query_csv = f"{query_id}_deepseek_hyperparameter.csv"
        query_heatmap = f"{query_id}_deepseek_hyperparameter_heatmap.png"
    ...
    if len(df) > 0:
        ...
        for temp in temperatures:
            for top_p in top_ps:
                ...
                for idx, row in df.iterrows():
                    query_id = row['query_id']
                    results_file = f"{query_id}_deepseek_hyperparameter.csv"
        ...
        if not summary_df.empty:
            summary_df.to_csv("deepseek_parameter_summary.csv", index=False)
            ...
            plt.tight_layout()
            plt.savefig("deepseek_summary_heatmap.png", dpi=300)
            plt.close()
            
            print("\nSummary of results saved to deepseek_parameter_summary.csv")
            print("Summary heatmap saved to deepseek_summary_heatmap.png")
